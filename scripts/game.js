...
define(['assets','messagebus','movements'],
    ...
    Game.prototype.deactivateGroup = function (name) {
        var i = this.groupnames.indexOf(name);
        if (i != -1) this.groupnames.splice(i,1);
    };

    Game.prototype.createGroup = function (name, members) {
        if (members.logicgroup) this.logicgroups[name] = members.logicgroup;
        if (members.gamewindows) this.gamewindows[name] = members.gamewindows;
        if (members.localmessagebusses) this.localmessagebusses[name] = members.localmessagebusses;
    };
